...
def ppdMakeModelSplit (ppd_make_and_model):
    ...
    elif l.startswith ("hewlett packard "):
        make = "HP"
        model = ppd_make_and_model[16:]
    elif l.startswith ("hewlett-packard "):
        make = "HP"
        model = ppd_make_and_model[16:]
    ...
    # Find the version number and truncate at this point. Truncate only
    # if the version number has only one digit or a dot with digits
    # before and after, to avoid removing model name parts (like "Epson
    # PX V500").
    model = re.sub (r" [vV](\d|\.?\d+\.\d+)(\D.*|)$", "", model)
    ...
    for suffix in [" hpijs",
        ...
        if s != -1:
            model = model[:s]
            modell = modell[:s]
    if make.lower () == "hp" or  make.lower () == "apollo":
        for suffix in [" printer",
                       " mfp",
                       " aio",
                       " multifunction",
                       " multifuntion",     # There are HP PPDs with this typo
                       " all in one",
                       " all-in-one",
                       " color capable",
                       " color inkjet",
    ...
    # Remove a "PS" at the end of the model name, to let printers which differ
    # only by the presence of a PostScript add-on appear as the same model
    model = re.sub (r"(?<!CU)(?<=[0-9A-Za-z])[ \-\_\,]*PS$", "", model)

    for mfr in [ "Apple", "Canon", "Epson", "Lexmark", "Oki" ]:
        if makel == mfr.lower ():
            make = mfr

    while make != "" and model.lower ().startswith (make.lower ()):
        model = model[len(make) + 1:]

    model = model.strip ()

    # Invalidate some bogus model names
    for m in ["cups",
              "gpl ghostscript",
              "esp ghostscript",
              "ghostscript"]:
...
DRIVER_TYPE_VENDOR_PS_PDL_MATCHED = 9
DRIVER_TYPE_VENDOR_PCL_PDL_MATCHED = 10
DRIVER_TYPE_VENDOR_PCL = 12
DRIVER_TYPE_VENDOR_PS = 13
DRIVER_TYPE_FOOMATIC_RECOMMENDED_POSTSCRIPT = 15
# Priority of hpcups temporary lowered, as hpcups has still regressions
# compared to hpijs (https://bugs.launchpad.net/hplip, all bugs with
# tag "hpcups"). Especially duplex does not work from most applications.
DRIVER_TYPE_HPCUPS = 18 # (16)
...
def _getDriverType (ppdname, ppds=None):
    ...
    if ppdname.startswith ("foomatic"):
        ...
        if (ppdname.find (":Generic")== -1 and
            ppdname.find ("Generic")!= -1):
    ...
    if ppdname.find ("-hpijs") != -1:
        if ppdname.find ("hpijs-rss") == -1:
            return DRIVER_TYPE_FOOMATIC_HPIJS
    # Vendor-supplied PCL PPD
    if (ppdname.find ("PXL") != -1 or
        ppdname.find ("PCL") != -1):
        return DRIVER_TYPE_VENDOR_PCL
    # Anything else should be a vendor's PostScript PPD.
    return DRIVER_TYPE_VENDOR_PS # vendor's own (PostScript)
...
class PPDs:
	...
	"""
        ...
        def getDriverTypeWithBias (x, mfg):
            ...
            if pdl_matched:
                # Usually we prefer the PCL PPD for a printer with both
                # PCL and PostScript support, to assure that it works if
                # an optional PostScript add-on is not installed.
                # But if or PPDs got selected by matching PDLs in the CMD:
                # fields of the device IDs of the PPD and of the detected
                # printer, we have a proof for whether the PostScript
                # add-on is installed and then we prefer PostScript.
                if t == DRIVER_TYPE_VENDOR_PCL:
                    t = DRIVER_TYPE_VENDOR_PCL_PDL_MATCHED
                if t == DRIVER_TYPE_VENDOR_PS:
                    t = DRIVER_TYPE_VENDOR_PS_PDL_MATCHED
            return t
	...
	"""
        ...
        pdl_matched = False;
        if id_matched:
            _debugprint ("Checking CMD fields")
            inexact = set()
            if commandsets:
                for ppdname in ppdnamelist:
                    ...
                    if not id:
                        #inexact.add (ppdname)
                        continue
                    # Fetch command set field
                    id_dict = parseDeviceID (id)
                    if not id_dict["CMD"] or len(id_dict["CMD"]) == 0:
                        #inexact.add (ppdname)
                        continue
                    # Check if one of the languages listed in the CMD: field
                    # in the PPD is not in the CMD: field of the auto-detected
                    # printer, the PPD will be considered a not so good choice
                    # and adde to the "inexact" list. This way PPDs where
                    # all languages in the CMD: field are also in the CMD:
                    # filed of the printer will be preferred. This will
                    # especially select the correct PPD if there is an
                    # optional PostScript add-on for a printer. See Red Hat
                    # bug #630058.
                    found = 0
                    for pdl in id_dict["CMD"]:
        ...
        for ppdname, ppddict in self.ppds.iteritems ():
            ...
            if (not ppdname.startswith("drv:///sample.drv/") and
                ...
                try:
                    ...
                    model = ''                
            lmake = make.lower ()
            if ppddict.has_key ('ppd-product-all'):
                model_list = ppddict['ppd-product-all']
            else:
                model_list = ()
            ...
            for m in model_list:
                if ppdmodelname == 0:
                    ...
                    if m.startswith ("(") and m.endswith (")"):
                        m = m[1:len (m) - 1]
                    if (not m.lower ().startswith (lmake) and
                        not m.lower ().startswith ("hewlett")):
                        m = make + " " + m;
                    (x, m) = ppdMakeModelSplit (m)
                ppdmodelname = 0
                if m == "":
                    continue
                lmodel = m.lower ()
                ...
                if not lmodels[lmake].has_key (lmodel):
                    lmodels[lmake][lmodel] = m
                    makes[make][m] = {}
                else:
                    m = lmodels[lmake][lmodel]

                #if not makes[make][m].has_key (ppdname):
                makes[make][m][ppdname] = ppddict
