...
from ubuntuone.syncdaemon.filesystem_manager import InconsistencyError
...
class Sync(object):
    ...
    def _handle_SV_FILE_DELETED(self, share_id, node_id):
    ...
    def handle_AQ_DELTA_OK(self, volume_id, delta_content, end_generation,
        ...
        for dt in delta_content:
            ...
            try:
                ...
                node_parent_id = self.m.fs.get_by_path(
                    self.m.fs.get_abspath(node.share_id,
    ...
    def handle_AQ_RESCAN_FROM_SCRATCH_OK(self, volume_id, delta_content,
        ...
        for node in all_nodes:
            ...
            if not node_id in live_nodes:
                self._handle_SV_FILE_DELETED(volume_id, node_id)
                deletes += 1
