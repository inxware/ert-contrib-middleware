...
class Volume(object):
    ...
    def __init__(self, volume_id, node_id, generation=None):
        """Create the volume."""
        # id and node_id should be str or None
...
class Share(Volume):
    ...
    def __init__(self, volume_id=None, node_id=None, path=None, name=None,
        ...
        """Create the share."""
        super(Share, self).__init__(volume_id, node_id, generation)
    ...
    def __repr__(self):
        return "<Share id %r, node_id %r, generation %r, active %r>" % \
                (self.volume_id, self.node_id, self.generation, self.active)
...
class UDF(Volume):
    ...
    def __init__(self, volume_id=None, node_id=None, suggested_path=None,
                 path=None, subscribed=True, generation=None):
        """Create the UDF, subscribed by default"""
        super(UDF, self).__init__(volume_id, node_id, generation)
...
class VolumeManager(object):
    ...
    def subscribe_udf(self, udf_id):
        ...
        try:
            ...
            d = defer.fail(VolumeDoesNotExist(udf_id))
        else:
            if not os.path.exists(udf.path):
    ...
    def handle_SV_VOLUME_CREATED(self, volume):
        ...
        if vol and vol.active:
            self.m.action_q.rescan_from_scratch(vol.volume_id)
