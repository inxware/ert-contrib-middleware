...
class FileSystemManager(object):
        ...
        def safeget_mdobj(mdid):
            ...
            try:
                ...
                try:
                    ...
                    log_warning('Share %r disappeared! deleting mdid: %s', mdobj['share_id'], mdid)
                    del self.fs[mdid]
                    return False
                else:
                    return mdid, mdobj
        safe_iteritems = itertools.imap(safeget_mdobj, self.fs.keys())
    ...
    def move_file(self, new_share_id, path_from, path_to):
        ...
        try:
            ...
            self.eq.rm_from_mute_filter(expected_event, path_from, path_to)
    ...
    def _delete_dir_tree(self, path, share_id):
        """Helper function to recursively remove a non-empty directory.

        Remove the dir if there aren't local changes, and return True.
        If there are, return False.

        Notifications are disabled for every sub-path within path.
        ...
        # all green
        for p, is_dir in subtree:
            filter_name = is_dir and "FS_DIR_DELETE" or "FS_FILE_DELETE"
            self.eq.add_to_mute_filter(filter_name, p)
            self.delete_metadata(p)

        # get the share and if it's read-only
        share = self._get_share(share_id)
        is_ro_share = not share.can_write()
        def handle_ro_share(func, subpath, exc_info):
            """Handle rmtree EACCES error for ro-shares."""
            if exc_info[0] == OSError and exc_info[1].errno == errno.EACCES \
               and is_ro_share:
                # use EnableShareWrite directly to avoid a lot of share
                # lookups.
                with EnableShareWrite(share, subpath):
                    func(subpath)
            else:
                raise
        shutil.rmtree(path, onerror=handle_ro_share)
        return True
    ...
    def delete_file(self, path):
        ...
        if self.is_dir(path=path):
            ...
            remove_action = os.rmdir
        else:
            expected_event = "FS_FILE_DELETE"
            remove_action = os.remove
        try:
            with self._enable_share_write(mdobj['share_id'], path):
                self.eq.add_to_mute_filter(expected_event, path)
                remove_action(path)
        except OSError, e:
            self.eq.rm_from_mute_filter(expected_event, path)
            if e.errno == errno.ENOTEMPTY:
                if not self._delete_dir_tree(path=path,
                                             share_id=mdobj['share_id']):
                    raise
            else:
    ...
    def move_to_conflict(self, mdid):
        ...
        with self._enable_share_write(mdobj['share_id'], path):
            try:
                ...
                self.eq.rm_from_mute_filter(expected_event, path, to_path)
        ...
        # search for given path plus the separator to avoid self, and
        # other similar paths that aren't *children*
    ...
    def delete_to_trash(self, mdid, parent_id):
        ...
        log_debug("delete_to_trash: mdid=%r, parent=%r, share=%r, node=%r, "
                  "path=%r", mdid, parent_id, share_id, node_id, path)
        self.delete_metadata(path)
        self.trash[(share_id, node_id)] = (mdid, parent_id, path)
    ...
    def get_iter_trash(self):
        ...
        for (share_id, node_id), node_info in self.trash.iteritems():
            yield share_id, node_id, node_info[1]
    ...
    def make_dir(self, mdid):
        ...
        with self._enable_share_write(mdobj.share_id, full_path) as enable:
            if not enable.ro:
                self.eq.add_to_mute_filter('FS_DIR_CREATE', full_path)
    ...
    def dereference_err_limbos(self, marker):
        ...
        """
        for (share, node), (_, parent, _) in self.trash.iteritems():
        ...
        for (share, node), (old_parent, new_parent, _) in move_items:
