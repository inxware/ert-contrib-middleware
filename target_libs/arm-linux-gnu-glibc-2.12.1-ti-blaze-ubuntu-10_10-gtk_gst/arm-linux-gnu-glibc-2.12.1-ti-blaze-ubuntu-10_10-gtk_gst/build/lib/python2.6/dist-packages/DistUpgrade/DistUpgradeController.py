...
class DistUpgradeController(object):
    ...
    def rewriteSourcesList(self, mirror_check=True):
        ...
        for entry in self.sources.list[:]:
            ...
            # special case for kde ppa (#680088)
            if (not entry.disabled and
                entry.uri.startswith("http://ppa.launchpad.net/kubuntu-ppa/backports/ubuntu")):
                logging.debug("changing ppa kde repo")
                entry.uri = "http://ppa.launchpad.net/kubuntu-ppa/ppa/ubuntu"
                entry.dist = self.toDist
                continue
