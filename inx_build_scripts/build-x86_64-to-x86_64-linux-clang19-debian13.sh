...
# Build contrib middleware for x86_64 Debian 13 (clang 19).
#
# Output: target_libs/x86_64-linux-gnu-clang19_debian13_base/
...
#
# Debian 13 provides opencv 4.10, ffmpeg 7.x, abseil, TBB etc. via apt.
# Only components absent from Debian 13 repositories are built here:
...
#
# AWS CRT version set — matches arm64-linux-gnu-clang19_debian13_base:
...
#
# Source tarballs must be downloaded and extracted into contrib/ before running.
# See the fetch-aws-crt-sources.sh helper script.

IMAGE_NAME=inxware/inx-debian13-clang19
...
OUTTARGET=x86_64-linux-gnu-clang19_debian13_base
PROCESSORS=$(nproc)
...
build_cmake() {
    local SRC="$1"; shift
    local EXTRA_ARGS="$*"
    local BUILD="${SRC}/build-debian13"
...
BUILD_DIR_C="${STAGING}/build/litert-x86_64-linux-c"
BUILD_DIR_CPP="${STAGING}/build/litert-x86_64-linux-cpp"
