...
INX_PROJECT_NAME=gtk_gst_test-to-delete
...
build_component libarchive -2.8.5 "--with-sysroot=${SYSROOT}"
...
build_component libXfont -1.4.4 #--enable-libxml2  
...
build_component ffmpeg -0.6 " --cross-prefix=${TOOLCHAIN_BIN_PREFIX} --enable-cross-compile  --sysroot=${SYSROOT} --arch=${ARCH} --target-os=linux --enable-version3 --enable-pthreads --disable-ffplay --disable-ffserver --disable-doc  --enable-libfaadbin --enable-postproc --enable-shared --enable-avfilter --enable-gpl --enable-nonfree --disable-libmp3lame --disable-libx264 --disable-libtheora --disable-libvorbis --enable-runtime-cpudetect --extra-cflags=-I${USRLIB_BUILD_ROOT}/include --extra-cflags=-L${USRLIB_BUILD_ROOT}/lib"  "NO_AUTOTOOLS_CROSS_COMPILE_HINTS"
...
##### ffmpeg is bteer now - drop build_component libmad -0.15.1b
