...
IMAGE_NAME=inxware/inx-debian11-clang-arm
echo "source 1"
...
#########################################################################################################
export CXXFLAGS="-v --target=aarch64-linux-gnu -I/usr/aarch64-linux-gnu/include/c++/10/aarch64-linux-gnu"
export CFLAGS="-v --target=aarch64-linux-gnu -I/usr/aarch64-linux-gnu/include/c++/10/aarch64-linux-gnu"
...
#this has deprecated stuff clang11 doesn't like in it so bumping up the version: build_component libarchive -3.0.0a "--with-sysroot=${SYSROOT}"
