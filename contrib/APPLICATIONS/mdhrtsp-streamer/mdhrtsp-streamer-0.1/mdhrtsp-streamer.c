...
static char gStreamPath[256]="/RAYMARINEMFD";
...
static void create_services(AvahiServer *s) {
   ...
   // if ((ret = avahi_server_add_service(s, group, AVAHI_IF_UNSPEC, AVAHI_PROTO_UNSPEC, 0, gStreamname, "_rtsp._tcp", NULL, NULL, gPortnumber, gStreamArgs[0], gStreamArgs[1], gStreamArgs[2], gStreamArgs[3], gStreamArgs[4], NULL)) < 0) {
    if ((ret = avahi_server_add_service(s, group, AVAHI_IF_UNSPEC, AVAHI_PROTO_INET, 0, gStreamname, "_rtsp._tcp", NULL, NULL, gPortnumber, gStreamArgs[0], gStreamArgs[1], gStreamArgs[2], gStreamArgs[3], gStreamArgs[4], NULL)) < 0) {
...
void * start_bonjour(void *nothing) {
    ...
    name = avahi_strdup("Raymarine");

    /* Let's set the host name for this server. */
    avahi_server_config_init(&config);
    config.host_name = avahi_strdup("RaymarineMFDServer");
	...
	printf("Text is %s\n",text);
   	while ( sscanf(ptr, "%255[^|]%n", field, &n) == 1 && args_read<5 )
   	{
		...
		if(valid_keyvaluepair==1){
			...
			if(strcmp(key,"raymarine-mfd-rtsp-path")==0){
			...
			//add this key value pair to the list of args for avahi
