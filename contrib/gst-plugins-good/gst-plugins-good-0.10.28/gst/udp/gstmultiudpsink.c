...
gst_multiudpsink_render (GstBaseSink * bsink, GstBuffer * buffer)
{
      ...
      while (TRUE) {
        ...
        if ( ret < 0) {
          ...
          if (!socket_error_is_ignorable ()) {
            ...
            g_free (errormessage);
            printf("****************** Exiting client with unrecoverable error\n *********************** \n");
            break;
          }
          else {
        	  printf("****************** Exiting client with recoverable error\n *********************** \n");
        	  //break; //move onto the next - don't want an infinite loop do we?
          }
        } else {
        	printf(".");
          num++;
  ...
  GST_LOG_OBJECT (sink, "sent %d bytes to %d (of %d) clients", size, num,
      no_clients);
  printf("++++++++++++++++++++++++++++++++++\nsent %d bytes to %d (of %d) clients+++++++++++++++++++++++\n", size, num,
      no_clients);
...
gst_multiudpsink_render_list (GstBaseSink * bsink, GstBufferList * list)
{
  ...
  while (gst_buffer_list_iterator_next_group (it)) {
    ...
    for (clients = sink->clients; clients; clients = g_list_next (clients)) {
      ...
      no_clients++;
      GST_LOG_OBJECT (sink, "sending %d bytes to client %p", size, client);
      printf("XXLISTXX - sending %d bytes to client %p", size, client);
      ...
      while (count--) {
        while (TRUE) {
          ...
          if (ret < 0) {
						if (!socket_error_is_ignorable()) {
							printf(
									"XXXX Exiting client with unrecoverable error\n");
							break;
						} else {
							printf(
									"XXXX Exiting client with recoverable error\n ");
							break; //move onto the next - don't want an infinite loop do we?
						}
					} else {
						num++;
    ...
    GST_LOG_OBJECT (sink, "sent %d bytes to %d (of %d) clients", size, num,
        no_clients);
    printf("XXXX sent %d bytes to %d (of %d) clients\n", size, num,
        no_clients);
  }
    ...
    const gchar * string)
{
  ...
  g_mutex_lock (sink->client_lock);
  printf(" * clear all existing clients * \n");
...
gst_multiudpsink_remove (GstMultiUDPSink * sink, const gchar * host, gint port)
{
  ...
  GST_DEBUG_OBJECT (sink, "found %d clients with host %s, port %d",
      client->refcount, host, port);
  printf("=========== found %d clients with host %s, port %d",
      client->refcount, host, port);
  client->refcount--;
...
gst_multiudpsink_clear_internal (GstMultiUDPSink * sink, gboolean lock)
{
  ...
  printf(" * Internal clear all existing clients * \n");
  if (lock)
    g_mutex_lock (sink->client_lock);
