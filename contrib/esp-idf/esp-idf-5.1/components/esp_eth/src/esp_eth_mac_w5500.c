...
#define W5500_SEND_CMD_TIMEOUT_MS 250
...
static esp_err_t emac_w5500_start(esp_eth_mac_t *mac)
{
    ...
    uint8_t reg_value = 0;
    /* open SOCK0 */
    ESP_GOTO_ON_ERROR(w5500_send_command(emac, W5500_SCR_OPEN, W5500_SEND_CMD_TIMEOUT_MS), err, TAG, "issue OPEN command failed");
...
static esp_err_t emac_w5500_stop(esp_eth_mac_t *mac)
{
    ...
    ESP_GOTO_ON_ERROR(w5500_write(emac, W5500_REG_SIMR, &reg_value, sizeof(reg_value)), err, TAG, "write SIMR failed");
    /* close SOCK0 */
    ESP_GOTO_ON_ERROR(w5500_send_command(emac, W5500_SCR_CLOSE, W5500_SEND_CMD_TIMEOUT_MS), err, TAG, "issue CLOSE command failed");
...
static esp_err_t emac_w5500_transmit(esp_eth_mac_t *mac, uint8_t *buf, uint32_t length)
{
    ...
    // issue SEND command
    ESP_GOTO_ON_ERROR(w5500_send_command(emac, W5500_SCR_SEND, W5500_SEND_CMD_TIMEOUT_MS), err, TAG, "issue SEND command failed");
...
static esp_err_t emac_w5500_receive(esp_eth_mac_t *mac, uint8_t *buf, uint32_t *length)
{
    ...
    /* issue RECV command */
    ESP_GOTO_ON_ERROR(w5500_send_command(emac, W5500_SCR_RECV, W5500_SEND_CMD_TIMEOUT_MS), err, TAG, "issue RECV command failed");
