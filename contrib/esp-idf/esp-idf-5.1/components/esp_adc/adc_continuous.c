...
IRAM_ATTR esp_err_t adc_continuous_clear_frame_in_ISR(adc_continuous_handle_t handle, size_t size, BaseType_t *pxHigherPriorityTaskWoken)
{
    ESP_RETURN_ON_FALSE(handle, ESP_ERR_INVALID_STATE, ADC_TAG, "The driver isn't initialised");
    ESP_RETURN_ON_FALSE(handle->fsm == ADC_FSM_STARTED, ESP_ERR_INVALID_STATE, ADC_TAG, "The driver is already stopped");
    ...
    data = xRingbufferReceiveUpToFromISR(handle->ringbuf_hdl, &_size, size);
    if (data != NULL) {
        vRingbufferReturnItemFromISR(handle->ringbuf_hdl, data, pxHigherPriorityTaskWoken);
    }
