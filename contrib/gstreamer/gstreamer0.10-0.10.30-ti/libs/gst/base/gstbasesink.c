...
{
  GstFlowReturn ret;
  printf("Entering gst_base_sink_queue_object\n");
...
gst_base_sink_event (GstPad * pad, GstEvent * event)
{
  ...
  GstBaseSinkClass *bclass;
printf("Entering gst_base_sink_event()\n");
    ...
    gboolean is_list, gpointer obj)
{
  ...
  GstBuffer *time_buf;
printf("Entering gst_base_sink_chain_unlocked\n");
  if (G_UNLIKELY (basesink->flushing))
    goto flushing;
  ...
  result = gst_base_sink_queue_object_unlocked (basesink, pad,
      is_list, obj, TRUE);
  printf("Exiting gst_base_sink_chain_unlocked -OK\n");
  return result;
...
flushing:
  {
    GST_DEBUG_OBJECT (basesink, "sink is flushing");
    gst_mini_object_unref (GST_MINI_OBJECT_CAST (obj));
    printf("Exiting gst_base_sink_chain_unlocked - GST_FLOW_WRONG_STATE\n");
    return GST_FLOW_WRONG_STATE;
  }
was_eos:
  {
    GST_DEBUG_OBJECT (basesink,
        "we are EOS, dropping object, return UNEXPECTED");
    gst_mini_object_unref (GST_MINI_OBJECT_CAST (obj));
    printf("Exiting gst_base_sink_chain_unlocked - GST_FLOW_UNEXPECTED\n");
    return GST_FLOW_UNEXPECTED;
  }
out_of_segment:
  {
    ...
    gst_mini_object_unref (GST_MINI_OBJECT_CAST (obj));
    printf("Exiting gst_base_sink_chain_unlocked - GST_FLOW_OK");
    return GST_FLOW_OK;
  }
...
{
  GstFlowReturn result;
  printf("Entering gst_base_sink_chain_main()\n");
...
printf("Exiting gst_base_sink_chain_main() - OK\n");
  return result;
...
wrong_mode:
  {
    ...
    printf("Exiting gst_base_sink_chain_main() - GST_FLOW_UNEXPECTED\n");
    goto done;
  }
}
...
gst_base_sink_chain_list (GstPad * pad, GstBufferList * list)
{
  ...
  GstBaseSinkClass *bclass;
  GstFlowReturn result;
  printf("Entering gst_base_sink_chain_list() \n");
  basesink = GST_BASE_SINK (GST_OBJECT_PARENT (pad));
  bclass = GST_BASE_SINK_GET_CLASS (basesink);
  ...
  printf("Exiting gst_base_sink_chain_list() \n");
  return result;
}
